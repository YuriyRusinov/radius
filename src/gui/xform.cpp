    ...
    : ArthurFrame(parent)
{
    ...
    m_pixmap = QPixmap(":res/radius/jellyfish.jpg");
    ...
    pts->setBoundingRect(QRectF(0, 0, 1000, 1000));
    ctrlPoints << QPointF(500, 500) << QPointF(600, 500);
...
void XFormView::setFile(const QString &name)
{
    if(name.isEmpty())
        return;
...
//read flt image in bytearray
       ...
       QByteArray datas;
     /*while (!file.atEnd()) {
         QByteArray line_ftl = file.readLine();
         datas = datas + line_ftl;
     }*/  
     
       datas = file.readAll();

     //image_flt = QImage::fromData(datas);

//


//        3480*5788*32 серый (*4 байт)
//      80.568.960 байт
//        32 бита = FLT=
//        32 бита = height * height (только файл image.flt)
//        size = 4*height*width
//        width = size/4*height

        int sz = datas.size();

        unsigned char heig_image[4];

        heig_image[0] = datas.at(4);
        heig_image[1]  = datas.at(5);
        heig_image[2] = datas.at(6);
        heig_image[3] = datas.at(7);
        
        int h_buf;
        char* bufC = (char*)&h_buf;
        ...
        int sh = 0;
        int sv = 0;
        // наши доморощенные файлы
        if(sz == 80568960)
        {
          sh = 3480;
          sv = 5788;
        }
        else if(sz == 571039480)
        {
          sh = 12451;
          sv = 8200;
        }
        ...
        {
//        else if (sz == 2097160) // только файл image.flt в правильном формате
//общий случай - полагаем что FLT-файл в правильном формате
        //h_buf = height*height//
        ...
        QImage image (sh, sv, QImage::Format_RGB32);
        ...
        unsigned int image_min;
        unsigned int image_d;

          //первые два 2-слова(по 32 бита) не берем - там служебная информация.
        mas[0] = datas.at(8);
        mas[1]  = datas.at(9);
        mas[2] = datas.at(10);
        mas[3] = datas.at(11);
        image_d = *((unsigned int * )( &mas[0] ));

        
        image_min = image_max = image_d;
          
        float loga;
        Q_UNUSED (loga);
        for(int j = 8; j < datas.size();)
        {
            mas[0] = datas.at(j);
            mas[1]  = datas.at(j+1);
            mas[2] = datas.at(j+2);
        ...
        //   loga = log10((float)image_d);
         //  image_d =(int) loga;

            if(image_d > image_max) image_max = image_d;
            if(image_d < image_min) image_min = image_d;
        ...
        float anton = log10((float)image_max) - log10((float)image_min);
        ...
        for(int i = 0; i < sh; i++)
        {
            for(int j= 0; j < sv; j++)
            {
                ...
                gray = *((unsigned int * )( &mas[0] ));


                // val = log10((float)gray);
                // gray = (int)val;

                val = 256.0 * ( gray - image_min)  / (image_d);

                val = 256.0 * (log10((float)gray) - log10((float)image_min)) / (float)anton;
...
{
    setWindowTitle(tr("Radius software sample transformations"));
    ...
    mainGroup->setTitle(tr("Transformations"));
...
//   connect(enableOpenGLButton, SIGNAL(clicked(bool)), view, SLOT(enableOpenGL(bool)));
    ...
    view->loadDescription(":res/radius/radius.html");

    // defaults
    view->reset();
//   pixmapType->setChecked(true);
    textEditor->setText("Radius software samples transformations");
    textEditor->setEnabled(false);

//   animateButton->animateClick();
}

void XFormWidget::setFile(const QString &name)
{
    view->setFile(name);
}
