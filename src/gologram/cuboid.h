...
std::vector<unsigned char> _FFTMATR_EXPORT_ createImageMatrix(const std::vector<Cuboid>&,const mslMesh::mesh3D&);//, QProgressDialog * pD=0);
std::vector<unsigned char> _FFTMATR_EXPORT_ createImage(const std::vector<unsigned char> &);

void _FFTMATR_EXPORT_ recurseCub(const std::pair<mslMesh::point3D<int, int, int>, mslMesh::point3D<int, int, int> > &,
                      const std::vector<Cuboid>&,
                      const std::vector<mslMesh::simpleFace3D>&,
                      std::vector<unsigned char>&,
                      int lineSize);

QImage _FFTMATR_EXPORT_ imageTransform(const std::vector<unsigned char>&, const std::string &);
